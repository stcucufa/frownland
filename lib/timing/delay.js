...
import { beginOf, cancelled, forward, min, pruned, Item, itemProperties } from "./util.js";

// Delay(duration) delays its value for `duration` amount of time, which is
// greater than zero. Cannot fail.
export const Delay = assign(createDelay, {
    tag: "Delay",
...
const DelayUntil = Item(["t"], extend(Delay, {
    tag: "Delay/until",
...
function createDelay(duration) {
    ...
    return assign(Object.create(Delay, properties), itemProperties);
}

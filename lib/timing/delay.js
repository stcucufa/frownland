import { extend, nop, parseTime, safe } from "../util.js";
import * as time from "./time.js";
import { Instant } from "./instant.js";

const proto = {
    tag: "Delay",

    show() {
        return `Delay(${time.show(this.duration)})`
    },

    generate(thread, t) {
        thread.timeline.push(extend(this, { pc: thread.ops.length, t }));
        const begin = t;
        const end = time.add(begin, this.duration ?? time.unresolved());
        if (time.isUnresolved(end)) {
            // Variable delay: get the duration from the thread value. Duration
            // must be definite and > 0. Keep track of the resolved duration
            // for redo.
            const resolvedDuration = [];
            thread.ops.push([
                (thread, vm) => {
                    if (resolvedDuration.length === 0) {
                        let duration = thread.value;
                        if (typeof duration === "string") {
                            duration = safe(parseTime)(duration);
                        }
                        if (!(typeof duration === "number" && duration > 0)) {
                            throw new Error("invalid duration for delay", { duration: thread.value });
                        }
                        resolvedDuration.push(duration);
                    }
                    vm.scheduleForward(thread, vm.t + resolvedDuration[0], end);
                },
                (thread, vm) => { vm.scheduleBackward(thread, vm.t - resolvedDuration[0]); }
            ]);
        } else if (this.duration > 0) {
            // Non-zero delay
            thread.ops.push([
                // DEBUG
                // (thread, vm) => { vm.scheduleForward(thread, vm.t + this.duration); },
                // nop
                (thread, vm) => {
                    const dest = vm.t + this.duration;
                    console.log(`Delay: ${vm.t} -> ${dest}`);
                    vm.scheduleForward(thread, dest);
                },
                () => { console.log("(arriving from delay/backward)"); }
            ]);
            thread.ops.push([
                // DEBUG
                // nop,
                // (thread, vm) => { vm.scheduleBackward(thread, vm.t - this.duration); }
                () => { console.log("(arriving from delay/forward)"); },
                (thread, vm) => { 
                    const dest = vm.t - this.duration;
                    console.log(`Delay: ${vm.t} <- ${dest}`);
                    vm.scheduleBackward(thread, dest);
                }

            ]);
        }
        return end;
    }
};

// Delay with an optional duration, which can be a number of milliseconds or a
// string that should parse to a time value
export const Delay = (...args) => args.length === 0 ? Object.create(proto) : extend(proto, {
    duration: time.check(args[0])
});

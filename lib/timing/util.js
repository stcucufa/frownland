...
export function inputParameters(instance) {
    ...
    if (instance.item.modifiers?.vars) {
        ...
        for (const item of instance.item.modifiers.vars) {
            const i = currentInstance(instance, item);
...
function currentInstance(instance, item) {
    ...
    for (let i = index - 1; i >= 0; --i) {
        const j = currentInstance(instance.parent.children[i], item, true);
        if (j) {
            return j;
        }
    }
    // Try children, then try parent.
    return instance.children?.find(j => currentInstance(j, item, true)) ??
        currentInstance(instance.parent, item);
}

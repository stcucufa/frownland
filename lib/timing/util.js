...
export function forward(t, interval) {
    ...
    const params = [];
    try {
        if (item.modifiers?.vars) {
            params.push(input);
            for (const it of item.modifiers.vars) {
                const i = currentInstance(instance, it);
                if (!i || !Object.hasOwn(i, "value")) {
        ...
        instance.value = item.valueForInstance.call(
            instance, params.length > 0 ? params : input, t, interval
        );
    } catch (error) {
        instance.error = error;
    }
...
// at it and its preceding siblings.
function currentInstance(instance, item) {
    ...
    for (let i = index - 1; i >= 0; --i) {
        const j = currentInstance(instance.parent.children[i], item);
        if (j) {
            return j;
        }
    }
    return currentInstance(instance.parent, item);
}

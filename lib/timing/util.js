import { assign, isNumber, parseTime, safe } from "../util.js";
...
// Create items, mapping parameters to properties in the created item.
export const Item = (params, proto) => assign(function (...args) {
    const item = Object.assign(Object.create(proto), itemProperties, zipo(params, args));
    item.init?.();
    return item;
}, proto);

// Create containers with children.
export const Container = proto => assign(function (...children) {
    const item = Object.assign(Object.create(proto), itemProperties, { children });
    item.init?.();
    return item;
}, proto);

// Zip an array of keys with an array of values into an object.
function zipo(keys, vals) {
    const o = {};

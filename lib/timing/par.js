import { extend, isObject, K, nop, push, values } from "../util.js";
                ...
                for (const name in this.children) {
                    t = generateChild(t, this.children[name], takeAny ? (childThread, vm) => {
                        ...
                        if (diff > 0) {
                            ...
                            if (takeSome && diff === 1) {
                                // The last item was added to the accumulator
                                // so the par is done and the rest of the
                                // children will be cancelled.
                                vm.wake(thread, hasDur ? end : vm.t);
                                childThread.lastChildOf = thread;
                            }
                        }

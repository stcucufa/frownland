        ...
        if (!empty) {
            ...
            const generateChild = (t, child, accumulate) => {
                ...
                childThread.do((childThread, vm) => {
                    ...
                    const state = childThread.parent.parState;
                    state.children.delete(childThread);
                    if (childCount - state.children.size === targetCount) {
                        // When enough children have ended, the parent thread
                        // can be awakened, and the remaining child threads are
                        // cancelled.
                        state.complete = true;
                        if (thread.parent) {
                            console.log(`!!! Wake ${thread.id}/${thread.subid} < ${thread.parent.id}/${thread.parent.subid}`);
                        } else {
                            console.log(`!!! Wake ${thread.id}/${thread.subid}`);
                        }
                        vm.wake(childThread.parent, hasDur ? state.begin + dur : vm.t);
                        for (const child of state.children) {
                            vm.cancelThread(child);
                        }
            ...
            if (isNamed) {
                ...
                for (const name in this.children) {
                    t = generateChild(t, this.children[name], childThread => {
                        childThread.parent.parState.accumulator[name] = childThread.value;
                    });
                }
            } else {
                ...
                t = this.children.reduce((t, child, i) => generateChild(
                    t, child, takeAny ? childThread => {
                        childThread.parent.parState.accumulator.push(childThread.value);
                    } : childThread => {
                        childThread.parent.parState.accumulator[i] = childThread.value;
                    }
                ), t);
            }

import { assign, extend, fold1, isNumber, partition, push, remove, single } from "../util.js";
import {
    dur, cancelled, ended, endOf, failed, forward, hasModifier, max, min, show, take, pruned,
    Container, Item, CancelError, InputError, FailureError
} from "./util.js";
...
export const Par = Container({
    ...
    map(g) {
        ...
        return Item(["g"], assign({}, this, ParMap))(g);
    },
...
// first(...) is a synonym for Par(...).take(1), but the value is also
// unwrapped (i.e., it is a single value, and not an array with a single
// element).
export const first = Container(assign({}, Par, {
    init() {
        this.take(1);
    },

    valueForInstance() {
        return single(Par.valueForInstance.call(this));
    },
}));

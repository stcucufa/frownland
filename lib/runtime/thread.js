...
export const Thread = () => extend(thread, { ops: [], timeline: [], values: [], t: 0, pc: 0, vp: 0 });
...
const threads = {
    ...
    scheduleForward(thread, t) {
        if (time.isDefinite(t)) {
            console.log(`>>> schedule forward @${t}`, thread);
            return this.futureQueue.insert(extend(thread, { t }));
        }
        return thread;
    },
    ...
    scheduleBackward(thread, t) {
        console.log(`<<< schedule backward @${t}`, thread);
        return this.pastQueue.insert(extend(thread, { t }));
    }
};

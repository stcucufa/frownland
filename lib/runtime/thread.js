...
export const Thread = () => extend(thread, { ops: [], timeline: [], values: [], t: 0, vp: 0 });
...
const threads = {
    ...
    scheduleForward(thread, t, pc) {
        if (time.isDefinite(t)) {
            console.log(`>>> schedule forward @${t}/${pc}`, thread);
            return this.futureQueue.insert(extend(thread, { t, pc }));
        }
        return thread;
    },
    ...
    scheduleBackward(thread, t, pc) {
        console.log(`<<< schedule backward @${t}/${pc}`, thread);

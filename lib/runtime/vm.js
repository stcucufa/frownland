...
const proto = {
    ...
    updateForward(from, to) {
        ...
        for (const [thread, ramp] of this.ramps.entries()) {
            ramp.f(clamp((to - ramp.begin) / ramp.dur, 0, 1), to);
        }
        delete this.forward;
    },
    ...
    // Register a ramp for a given duration. This also schedules the thread
    // forward.
    registerRamp(thread, f, dur) {
        console.assert(!this.ramps.has(thread));
        this.ramps.set(thread, { f, begin: this.t, dur });
        this.schedule(thread, this.t + dur);
    },

    // Unregister the ramp once it is done.
    unregisterRamp(thread) {
        console.assert(this.ramps.has(thread));

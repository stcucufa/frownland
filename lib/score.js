...
export const Instant = assign(f => f ? extend(Instant, { valueForInstance: f }) : Object.create(Instant), {
    ...
    // If this follows an Instant or an effect, these two should be combined.
    followingInPar(previous) {
        if (previous?.tag === "Instant" || previous?.tag === "Instant/par") {
            return [InstantPar(previous, this)];
        }
        if (previous?.tag === "Effect" || previous?.tag === "Effect/par") {
            return [EffectPar(previous, this)];
        }
...
export const Par = assign(createPar, {
    tag: "Par",
    show,
    repeat,
    take,
...
// Simplify children for a Par.
function createPar(children) {
    if (!(children?.length > 0)) {
        return;
    }

    const simplifiedChildren = children.reduce(function (children, child) {
        if (!child) {
            // This child has been simplifed away.
            return children;
        }
        const i = children.length - 1;
        const [previous, current] = child.followingInPar(children[i]);
        if (!current) {
            // This child has been combined with the previous (e.g., two
            // Instants in a row) so we remove the previous one to replace
            // it with the new combined one.
            children[i] = previous;
        } else {
